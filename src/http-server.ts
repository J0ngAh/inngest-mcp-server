  ...
  try {
    ...
    if (sessionId && transports[sessionId]) {
      ...
      transport = new StreamableHTTPServerTransport({
        ...
        // Security: Enable DNS rebinding protection for production
        enableDnsRebindingProtection: false, // Disabled for local development
        allowedHosts: ['127.0.0.1', 'localhost'], // Only allow localhost
      });
...
process.on('SIGINT', () => {
  console.log('\nShutting down Inngest Event Manager HTTP Server...');
  process.exit(0);
});
